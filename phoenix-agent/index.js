...
async function backendPost(backendUrl, path, body, agentKey = '') {
  ...
  const opts = { method: 'POST', headers, signal: AbortSignal.timeout(15000) };
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Backend ${path} [${resp.status}]: ${detail || resp.statusText}`);
  }
  return resp.json();
}
    ...
    execute: async ({ migration_id, jetton_address, amount, min_ton_out = 0, backend_url = 'http://localhost:8000', agent_key = '' }) => {
      try {
        ...
        for (let i = 0; i < NUM_TRADES; i++) {
          let thisAmount;
          if (i < NUM_TRADES - 1) {
            // Trades 1-3: fixed tranche size from initial balance
            thisAmount = trancheSize;
          } else {
            // Trade 4: re-read wallet and sell whatever remains
            const remaining = await sdk.ton.getJettonBalance(jetton_address);
            sdk.log.info(`Final trade: wallet has ${remaining} tokens remaining`);
            thisAmount = remaining;
          }

          if (thisAmount <= 0) {
            sdk.log.info(`Nothing to sell on trade ${i + 1} — skipping`);
            continue;
          }
